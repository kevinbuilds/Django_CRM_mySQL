...
def home(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return redirect('home')
    # if not posting username or password, user is just visiting the page, hence the rendering of it.
    else:
...
def delete_record(request, pk):
    if request.user.is_authenticated:
        ...
        delete_it.delete()
        messages.success(request, "Record {a} Deleted Successfully".format(a=pk))
        return redirect('home')
    else:
        messages.success(request, "You have to be authenticated to perform this action. ")
        return redirect('home')
